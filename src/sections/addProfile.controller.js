...
class AddProfileController {
    constructor(Upload, $http, $timeout, $window, $firebaseArray, $firebaseObject, CONFIG, $state) {
        ...
        let username = 'mailer-daemon';
        let password = 'valarmorghulisisthebestpasswordever';
        let authdata = btoa(username + ':' + password);
        this.$http.defaults.headers.common['Authorization'] = 'Basic ' + authdata; // eslint-disable-line
    }
    ...
    uploadPhoto(file) {
        ...
        let sha1 = new jsSHA("SHA-1", "TEXT"); //eslint-disable-line
